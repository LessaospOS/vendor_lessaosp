...
cat <<EOF
Additional LessaospOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- lessaospgerrit:    A Git wrapper that fetches/pushes patch from/to LessaospOS Gerrit Review.
- lessaosprebase:    Rebase a Gerrit change and push it again.
- lessaospremote:    Add git remote for LessaospOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for LessaospOS Github.
...
function lessaospremote()
{
    ...
    then
        ...
        local PFX="LessaospOS/"
    else
        local PROJECT=$REMOTE
    fi

    local LESSAOSP_USER=$(git config --get review.review.lessaospos.org.username)
    if [ -z "$LESSAOSP_USER" ]
    then
        git remote add lessaosp ssh://review.lessaospos.org:29418/$PFX$PROJECT
    else
        git remote add lessaosp ssh://$LESSAOSP_USER@review.lessaospos.org:29418/$PFX$PROJECT
    fi
...
function lessaospgerrit() {
    ...
    local user=`git config --get review.review.lessaospos.org.username`
...
function lessaosprebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "LessaospOS Gerrit Rebase Usage: "
    ...
    git fetch "http://review.lessaospos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
